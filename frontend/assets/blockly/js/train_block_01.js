        ...
        // Detect the phone/PC's actual local subnet via a WebRTC ICE candidate,
        // so discovery works on ANY Wi-Fi network, not just a hardcoded list.
        ...
        async function isBackendAlive(url, timeoutMs = 1200) {
            try {
                const controller = new AbortController();
                const timeoutId = setTimeout(() => controller.abort(), timeoutMs);
                await fetch(url + '/health', { method: 'GET', mode: 'no-cors', signal: controller.signal });
        ...
        async function discoverBackend() {
            ...
            if (manualUrl && !manualUrl.includes('localhost') && !manualUrl.includes('127.0.0.1')) {
                ...
                // Only trust a manual/previously-found URL if it's still reachable —
                // otherwise a stale IP from a different Wi-Fi network blocks rediscovery forever.
                if (await isBackendAlive(url)) {
            ...
            if (dynamicBackendUrl) {
                if (await isBackendAlive(dynamicBackendUrl)) return dynamicBackendUrl;
            ...
            if (saved && !saved.includes('localhost') && !saved.includes('127.0.0.1') && await isBackendAlive(saved)) {
            ...
            // Scan the phone's actual current subnet first (works on any network),
            // then fall back to the known office/lab subnets as a backup.
            ...
            if (detectedSubnet && !subnets.includes(detectedSubnet)) {
                subnets = [detectedSubnet, ...subnets];
            } else if (detectedSubnet) {
                subnets = [detectedSubnet, ...subnets.filter(s => s !== detectedSubnet)];
            }
            ...
            // Probe with bounded concurrency — firing all ~1500+ candidates at once
            // exhausts the WebView's socket pool (ERR_INSUFFICIENT_RESOURCES) and the
            // scan fails outright instead of finding a reachable host.
            ...
            return new Promise((resolve) => {
                ...
                function launchNext() {
                    ...
                    if (idx >= candidates.length) {
                        if (active === 0 && !found) resolve(null);
                        return;
                    }
                ...
                for (let k = 0; k < CONCURRENCY; k++) launchNext();
            }).finally(() => {
                overlay.remove();
            });
        }
...
async function toggleK230TestCamera() {
    ...
    if (img) {
        ...
        img.src = ${backendUrl}/k230_stream?t=;
    }
    if (btn) btn.textContent = 'Stop K230';
}
...
async function toggleEsp32TestCamera() {
    ...
    if (img) {
        img.style.display = 'block';
        const backendUrl = getBackendUrl();
        img.src = ${backendUrl}/esp32_stream?t=;
    }
    if (btn) btn.textContent = 'Stop ESP32';
}
...
async function resetK230Board() {
    ...
    try {
        const backendUrl = getBackendUrl();
        await fetch(${backendUrl}/reset_k230, { method: 'POST' }).catch(() => null);
        alert('Reset signal sent to K230 board.');
    } catch (err) {
        alert('K230 Reset error: ' + (err.message || err));
    } finally {
...
async function resetEsp32Board() {
    ...
    if (modal) {
        ...
        alert('Please press and release the physical RST button on your ESP32 board.');
    }
}

async function exportTeachableMachineZip() {
    if (typeof exportArduinoZip === 'function') {
        return exportArduinoZip();
    }
}

async function exportMobileNetV3Tflite() {
    if (typeof exportTflite === 'function') {
        return exportTflite();
    }
}
